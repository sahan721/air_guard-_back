import { getLatestForecast, getForecastHistory, refreshForecast, } from "../services/weather.service.old.js";
function parseDeviceId(deviceIdParam) {
    if (!deviceIdParam) {
        return null;
    }
...
export async function getLatestForecastHandler(req, res) {
    try {
...
export async function getForecastHistoryHandler(req, res) {
    try {
        const deviceId = parseDeviceId(req.params.deviceId);
        if (deviceId === null) {
            return res.status(400).json({
...
export async function refreshForecastHandler(req, res) {
    try {
        const deviceId = parseDeviceId(req.params.deviceId);
        if (deviceId === null) {
            return res.status(400).json({
